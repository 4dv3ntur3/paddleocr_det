...
class DBPostProcess(object):
    ...
    def boxes_from_bitmap(self, pred, _bitmap, dest_width, dest_height, image_pure_name):
        ...
        src_img = cv2.cvtColor((bitmap * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR)
        ...
        for index in range(num_contours):
            ...
            if self.box_thresh > score: # 다음 box 
                continue
    ...
    def get_mini_boxes(self, contour, src_img=None):
        ...
        # cv2.imwrite('./draw_bbox_{}.jpg'.format(self.counter), src_img)
    ...
    def __call__(self, outs_dict, shape_list, image_pure_name): # preds, shape_list
        ...
        # new_mask = cv2.adaptiveThreshold(segmentation_, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 15, 10)
        ...
        for batch_index in range(pred.shape[0]):
            ...
            ### 여기서 threshold 한 번 더 적용? 
            # mask_ = (mask*255).astype(np.uint8)
            
            # from PIL import Image
            # im = Image.fromarray((new_mask))
            # im.save('./sample_result_mask_adaptive_thresh_cv2_{}.jpg'.format(self.counter))
